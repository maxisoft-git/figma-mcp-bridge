...
- **`server/`** — Node.js MCP server (`@gethopp/figma-mcp-bridge`). Published to npm. Runs via stdio transport. Uses `@modelcontextprotocol/sdk`, `ws`, `zod`. Leader/follower election allows multiple instances.
