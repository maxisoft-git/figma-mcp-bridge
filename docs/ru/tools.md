...
111 инструментов, сгруппированных по назначению. Каждый инструмент:
...
| Tool | Эффект |
|---|---|
| `batch_mutation` | Много мелких мутаций в одном вызове (быстрее, чем 111 отдельных) |
...
Помимо базового набора выше, мост содержит расширенный набор, портированный из
upstream и форка `realSeyed/figma-mcp-bridge` (MIT):

| Инструмент | Что делает |
|---|---|
| `get_layout_tree` | Абсолютные трансформации и границы всех нод под корнем захвата (геометрия только для чтения, отдельно от скриншотов). |
| `create_page` | Создать страницу, опционально задать имя и переключиться на неё. Возвращает ID для `parentId`. |
| `import_html_layers` | Импортировать JSON-дерево слоёв html-figma `htmlToFigma()` как редактируемые слои в новом фрейме-обёртке (фреймы, текст, прямоугольники, SVG). Источник — JSON-файл внутри cwd сервера. |
| `execute_code` | Выполнить JavaScript в песочнице плагина через Figma Plugin API — универсальная отмычка для того, что не покрыто остальными инструментами. Записи реальны. |
| `list_sections` / `get_section` | Список секций (страница, родительская секция, число детей) и чтение одной (дети, реальные границы содержимого, выход за края). |
| `create_section` | Создать секцию: пустую заданного размера или обернуть существующие ноды, не сдвигая их. |
| `fit_section` | Подогнать секцию вплотную к видимым детям. |
| `move_to_section` / `move_out_of_section` | Перенести ноды в секцию или поднять их на уровень выше, сохраняя позицию на холсте. |
| `update_variable_collection` / `delete_variable_collection` | Переименовать или удалить коллекцию переменных. |
| `create_variables` | Создать до 200 переменных за вызов, включая алиасы на переменные того же батча. |
| `update_variables` | Переименовать переменные и менять значение, scopes или описание. |
| `delete_variables` | Удалить переменные и сообщить, какие другие переменные на них ссылались. |
| `bind_variables` | Привязать переменные к полям ноды (fills, strokes, размер, padding, текст, …) или снять привязку. |
| `list_fonts` | Список доступных семейств и стилей шрифтов. |
| `update_text_style` / `delete_text_style` / `apply_text_style` | Изменить, удалить или применить локальный текстовый стиль (опционально к диапазону символов). |
| `get_component` / `get_instance` | Подробно прочитать компонент, набор компонентов или инстанс. |
| `combine_as_variants` | Объединить свободные компоненты в набор компонентов (варианты). |
| `swap_instance` / `detach_instance` | Заменить главный компонент инстанса или отсоединить его. |
| `add_component_property` / `edit_component_property` / `delete_component_property` / `bind_component_property` | Управление свойствами компонентов и их привязками. |

Для совпадающих имён базовые инструменты сохраняют свою реализацию
