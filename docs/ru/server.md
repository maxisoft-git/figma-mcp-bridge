...
MCP-сервер — это **мост между AI-агентом и Figma-плагином**. Реализует [Model Context Protocol](https://modelcontextprotocol.io/) (stdio transport) и экспонирует 111 инструментов для работы с Figma. Также запускает HTTP + WebSocket-сервер на `localhost:1994`, к которому подключается Figma-плагин.
...
Инструменты экспорта принимают `PNG`, `SVG`, `JPG`, `PDF` и `WEBP` (`server/src/export-format.ts`).

- Figma `exportAsync` не умеет webp, поэтому запрос `WEBP` экспортируется как PNG
  и перекодируется сервером через `cwebp` (libwebp) — бинарник должен быть в
  `PATH` (`server/src/webp.ts`). Если его нет, в ошибке указана команда установки.
- `save_screenshots` и `get_image` определяют формат по расширению `outputPath`,
  если `format` не задан (`.png`, `.svg`, `.jpg`/`.jpeg`, `.pdf`, `.webp`). Если
  `format` противоречит расширению, вызов падает — файл не получит несоответствующие байты.
- Чтение `get_node` ограничено 50 000 символов; большее поддерево обрезается
  по нодам на стороне плагина (`serializeNodeWithinBudget`) и возвращает
  `truncated`, `note` и `childCount` у нод, на которых обход остановился.
